"""
Script to get list of all problems by parsing readme.md from all subdirectories under ..\\repositories\\
...
def create_problems_list(files):
    ...
    for item in files.items():
        ...
        full_link = get_link(link)
        data[PROBLEM_LINK_STRING] = full_link
...
def get_link(local_path):
    local_path_parts = local_path.split('/')
    repo_name = local_path_parts[1]
    repo_path = local_path.replace(
        'repositories',
        'https://github.com/computer-science-engineering').replace(
            repo_name, f'{repo_name}/tree/master')
    return repo_path
